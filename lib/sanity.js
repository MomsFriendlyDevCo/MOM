	...
	*/
	formatResponse(id, rawResponse) {
		...
		if (typeof rawResponse == 'string') { // Accept '(OK|WARN|ERROR): (message)' style responses
			...
			return rawResponse.map((response, responseOffset) =>
				this.formatResponse(responseOffset, response)
			);
			...
			Object.assign(response, {
				...
				metrics: (
					...
					.map((metric, metricOffset) => ({
						...
						unit: 'number',
						...metric,
						id: metric.id ? `${response.id}.${metric.id}` : `${response.id}.${metricOffset}`
					}))
			});
		} else {
		...
		return this.promise()
			...
			.then(responses => {
				...
				return Promise.all(this.#reporters.map(reporter =>
					Promise.resolve()
						.then(()=> reporter.reporter.run.call(this, {responses, metrics: responses.flatMap(r => r.metrics)}, reporter.options))
