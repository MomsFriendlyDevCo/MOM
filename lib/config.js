...
let configArgs = program
	.option('-c, --config <path>', 'Specify the .env files to read instead of the defaults (CSV string)')
...
export let env = new DotEnv()
	.parse(configArgs.config
		? configArgs.config.split(/\s*,\s*/)
		...
		{
			allowMissing: !configArgs.config,
		}
	)
	.tap(dotenv => console.log('CONF', dotenv.value()))
	.schema({
	...
	.thru(dotenv => {
		...
		configPreTemplate = dotenv.value({clone: true});

		dotenv.template({ // Apply template using both the pre template values and the tree structure
