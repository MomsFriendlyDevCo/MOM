...
export default class MOMResponseMetric {
	...
	* @param {Number} value The current value to examine
	* @param {String} expr The value expression to evaluate
	* @param {Number} [max] Optional maximum value, if known
	* @returns {Boolean} A boolean true if the expression matches
	*/
	...
	static statusFromMetric(metric) {
		try {
			if (
				...
				&& metric.critValue
				&& MOMResponseMetric.evalExpression(metric.value, metric.critValue)
			) {
				return 'CRIT';
			} else if (
				metric.value !== undefined
				&& metric.warnValue
				&& MOMResponseMetric.evalExpression(metric.value, metric.warnValue)
			) {
				return 'WARN';
			} else {
				return 'PASS'
