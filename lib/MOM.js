...
export class MOM { // NOTE: We use Eventer.extend() instead of direct inheritence as we only need some functions
	...
	* @property {Object} serverTags Default tags to apply to all output
	*/
	settings = {
		paranoid: true,
		serverId: process.env.HOSTNAME,
		serverTitle: process.env.HOSTNAME,
		serverTags: {Server: process.env.HOSTNAME},
	...
	* @property {MOMResponse} [lastResponse] Last module response, if any
	* @property {Promise} readyPromise The promise which completes when `init()` has completed
	*/
	#reporters = [];
	...
	* @param {Boolean} [options.base=true] Accept basic config into the MOM instance
	...
	*/
	loadFromConfig(options) {
		...
		if (settings.base)
			Object.assign(this.settings, settings.config.settings);
			...
			newModule.readyPromise = import(module) // NOTE: Loading in background, only ready when `isReady: true`
				...
				.then(configRes => {
					if (configRes instanceof DotEnvSchema) { // Got back a DotEnvSchema
						try {
							newModule.options = {
								...configRes.apply(newModule.options),
								tags: newModule.options.tags || {},
							};
						} catch (e) {
				...
				.then(configRes => {
					if (configRes instanceof DotEnvSchema) { // Got back a DotEnvSchema
						try {
							...
							console.log('Given options:', newReporter.options);
							throw new Error(`Loading REPORTER:${newReporter.id}.config() - ${e.toString().replace(/^Error: /, '')}`, {cause: e});
						}
					} else {
	...
	* This adds various fluff and formats various fields into the {MOMResponse} format
	...
	*/
	formatResponse(id, rawResponse) {
		...
		if (!rawResponse) {
		...
		} else if (typeof rawResponse == 'string') { // Accept '(PASS|WARN|ERROR): (message)' style responses
			let {status, message} = /^(?<status>PASS|WARN|CRIT|ERROR):\s*(?<message>.*)$/?.groups || {};
			if (!status) throw new Error('Unprocessable MOM module string response');
			Object.assign(response, {status, message});
		} else if (Array.isArray(rawResponse)) {
			throw new Error('A module can only return one MOMResponse - use multiple metrics to respond to different payloads');
			...
			Object.assign(response, {
				...
				tags: {
					...this.settings.serverTags,
					...(rawResponse.tags || this.#modulesById[response.id].options.tags),
				},
				metrics: (
		...
		// Validate response
		if (this.settings.paranoid) MOMResponse.validate(response);
	...
	* @returns {MOMResponse} The formatted module response
	*/
	run(id) {
		...
		return this.promise()
			...
			.then(responses => ({
				...
				metrics: responses
					...
					.flat()
					.filter(Boolean),
				maxStatus: MOMResponseMetric.maxStatus(responses.map(r => r.status)),
			}))
				...
				return Promise.all(this.#reporters.map(reporter =>
					Promise.resolve()
						...
						.then(moduleResponse => { // Stash output if it returns something non-falsy
							...
							if (this.settings.paranoid) MOMResponse.validate(moduleResponse);
